...
Bonus: the \lstinline{todonotes} package also provides cool way to leave notes to yourself in a document using \lstinline+\todo{insert note}+ or \lstinline+\todo[inline]{insert note}+. \todo{note here} \todo[inline]{Inline note}
...
% temporary turn off lstset language to get comments to appear well
\lstset{language = }
...
\begin{minipage}{\linewidth}
\begin{lstlisting}
  \includegraphics[width = .9\textwidth]{imgs/attack-defense.pdf}
\end{lstlisting}
\end{minipage}

\ldots specifies that we want to include the image called \lstinline{attack-defense.pdf}, which is located in the \lstinline{imgs} folder. We specify the width to be 90 percent of the width of the text block, but we could have specified a width in inches or centimeters. The height is scaled automatically with the width.
...
\begin{itemize}
  ...
  \item The {\LaTeX} Wikibook. This is how I (Michael DeCrescenzo) learned {\LaTeX}. \url{https://en.wikibooks.org/wiki/LaTeX}
